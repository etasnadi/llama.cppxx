...
__global__ void __launch_bounds__(WG_SIZE, 1) mm(uint          K,
    ...
    if(threadIdx.x == 0 && blockIdx.x == 0 && blockIdx.y == 0){
        /*
        printf("Kernel call:\n");
        printf("K: %d, NPQ: %d, CRS: %d -- ", K, NPQ, CRS);
        printf("Cout: %d; N: %d, OW: %d, OH: %d; Cin: %d, KW: %d, KH: %d -- ", 
            dp.Cout, dp.N, dp.OW, dp.OH, dp.Cin, dp.KW, dp.KH);
        printf("W: %d, H: %d, s0: %d, p0: %d, d0: %d, s1: %d, p1: %d, d1: %d\n", 
            dp.W, dp.H, dp.s0, dp.p0, dp.d0, dp.d1, dp.p1, dp.d1);
        */
    }
